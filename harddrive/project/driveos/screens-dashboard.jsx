...
function cardShell(title, icon, iconColor, right, body, extraClass) {
  return h("div", { className: "card fade-up " + (extraClass || "") },
    h("div", { className: "card-head" },
      icon && h(Icon, { name: icon, size: 16, style: { color: iconColor || "var(--tx-mut)" } }),
...
function OffendersCard({ go }) {
  const offenders = [...window.DB.projects].sort((a, b) => b.sizeTB - a.sizeTB).slice(0, 5);
...
function ReadyArchiveCard({ go }) {
  const list = window.DB.projects.filter((p) => p.status === "ready" || p.archiveReady >= 80);
...
// ---- Team storage ----
function TeamCard() {
  const team = window.DB.team.filter((m) => m.usedTB > 0).sort((a, b) => b.usedTB - a.usedTB);
